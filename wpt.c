...
int main(int argc, char* argv[]){
   ...
   fprintf(stderr,"WPT - Web Pressure Test "PROGRAM_VERSION"\n"
     "Copyright (c), GPL Open Source Software.\n"
   );
...
void build_request(const char *url){
    ...
    if (proxyhost == NULL){
       if (index(url+i,':') != NULL && index(url+i,':') < index(url+i,'/')){
          ...
          bzero(tmp,10);
          strncpy(tmp,index(url+i,':')+1,strchr(url+i,'/')-index(url+i,':')-1);          //E2
          proxyport = atoi(tmp);
          
          if (proxyport == 0) proxyport = 80;      
       } else {
           strncpy(host,url+i,strcspn(url+i,"/"));                                  //E3
       }
